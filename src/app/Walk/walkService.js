...
exports.createReview = async function (userId, walkId, reviewContent) {
  try {
    ...
    const reviewPostResult = await walkDao.insertReview(
      connection,
      insertReviewParams
    );
...
// 피드백 생성
exports.createFeedback = async function (walkId, userId, reviewContent) {
  try {
    const insertFeedbackParams = [walkId, userId, reviewContent];

    const connection = await pool.getConnection(async (conn) => conn);

    const feedbackPostResult = await walkDao.insertFeedback(
      connection,
...
exports.editReview = async function (reviewId) {
  try {
    ...
    const connection = await pool.getConnection(async (conn) => conn);
    const editReviewResult = await walkDao.updateReview(
      connection,
      updateReviewParams
    );
...
exports.minusReview = async function (reviewId) {
  try {
    ...
    const connection = await pool.getConnection(async (conn) => conn);
    const minusReviewResult = await walkDao.updateMinusReview(
      connection,
      minusReviewParams
    );
...
exports.removeReview = async function (reviewId) {
  try {
    ...
    const connection = await pool.getConnection(async (conn) => conn);
    const deleteReviewResult = await walkDao.deleteReview(
      connection,
      deleteReviewParams
    );
    //connection.release();
...
exports.editFeedback = async function (feedbackId) {
  try {
    ...
    const connection = await pool.getConnection(async (conn) => conn);
    const editFeedbackResult = await walkDao.updateFeedback(
      connection,
...
exports.minusFeedback = async function (feedbackId) {
  try {
    ...
    const connection = await pool.getConnection(async (conn) => conn);
    const minusFeedbackResult = await walkDao.minusFeedback(
      connection,
      minusFeedbackParams
    );
...
exports.createReport = async function (userId, feedbackId, reportType) {
  try {
    ...
    const reportPostResult = await walkDao.insertReport(
      connection,
      insertReportParams
    );
